...
Отдельно проверено на 5 реальных документах (docx/pptx/pdf, положены в
`data/raw/`): NLP-пайплайн (`backend/nlp_pipeline/`) корректно проходит все
этапы — извлечение текста (включая OCR для сканов), определение языка по
абзацам, чанкинг — на всех пяти файлах без ошибок. Однако **из этой
песочницы заблокирован исходящий трафик на любые внешние LLM API**
(проверено: GigaChat, OpenAI, Groq, Together, DeepSeek — везде HTTP 403 на
CONNECT; доступен только `api.anthropic.com`), поэтому этап LLM-экстракции
в этой сессии не мог быть прогнан вживую на реальном ключе. Механика
подтверждена юнит-тестом с подменённым (mocked) ответом LLM — см.
`backend/nlp_pipeline/eval.py` и `data/golden/` — и `data/real_graph.json`
содержит только узлы Publication (без ошибок пайплайна, просто без
LLM-вызова). **Чтобы получить реальный граф — запустите пайплайн локально**
(см. «NLP-пайплайн» ниже), там сетевых ограничений нет.
...
- **NLP-пайплайн** (`backend/nlp_pipeline/`) — сырой документ (docx/pptx/pdf) →
  ...
  9. `eval.py` + `data/golden/*.json` — 5 вручную размеченных примеров из
     реальных загруженных документов (см. `data/raw/`), precision/recall/F1
     по сущностям и связям с fuzzy-сопоставлением имён.

  ```bash
  export GIGACHAT_API_KEY=...   # или LLM_API_BASE/LLM_API_KEY/LLM_MODEL для любого OpenAI-совместимого API
  ...
  python -m backend.nlp_pipeline.eval data/golden   # precision/recall на golden-наборе
...
| NLP-пайплайн извлечения сущностей из сырых документов (RU/EN) | ⚠ построен и протестирован (`backend/nlp_pipeline/`, docx/pptx/pdf+OCR, chunking по языку, entity resolution, валидация, confidence-эвристика, golden-набор с P/R/F1), но не прогнан вживую на реальном ключе — из песочницы блокируются все внешние LLM API (проверено 5 провайдеров, см. выше) |
