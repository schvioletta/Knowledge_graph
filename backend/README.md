...
- **RAG-чат по загруженным документам** (`backend/rag/`) — отдельный от
  графа знаний контур: пользователь загружает файл (`.pdf/.docx/.txt`) или
  ссылку на статью через UI (панель «База знаний для чата» под строкой
  поиска), документ чанкуется тем же алгоритмом, что и NLP-пайплайн
  (`nlp_pipeline/chunking.py`), чанки кодируются локальной моделью
  `sentence-transformers` (`paraphrase-multilingual-MiniLM-L12-v2`, RU/EN,
  без внешнего сервера векторной БД — косинусный поиск в numpy, демо-масштаб)
  и сохраняются в `data/rag/` (`store.py`). Вопрос из той же строки поиска
  параллельно уходит и в граф (`/api/search`), и в RAG (`/api/rag/ask`) —
  это два независимых источника ответа на разных вкладках («Текстовый
  ответ» vs «По документам»), не подменяющих друг друга.
  - Отказ вместо выдумки: если `store.search()` не находит фрагмент выше
    ...
    (`qa.py`) — иначе модель почти гарантированно ответила бы из общих
    знаний, а не из документа.
  - Промпт требует ссылку `[N]` на конкретный фрагмент под каждым
    ...
    Без LLM (`YANDEX_API_KEY` не задан) — честный детерминированный фолбэк:
    сами топ-фрагменты с указанием документа и страницы/абзаца, а не тишина.
  - Confidence — та же логика, что `infer_confidence` в `graph_writer.py`:
    ...
    отсутствии совпадений — отдельное значение «нет данных» (не путается с
    «низкая», это разные сигналы: «нашли, но неуверенно» vs «не нашли
    ничего»).
  - Ссылка на статью: PDF по прямому URL или HTML (`rag/ingest_link.py`,
    `BeautifulSoup`, скрипты/навигация вырезаются, заголовок страницы —
    заголовок источника).
  - Эндпоинты: `POST /api/documents/upload` (multipart), `POST
    /api/documents/link` (`{"url": "..."}`), `GET /api/documents`, `GET
    /api/rag/ask?q=...`.
  - Проверено вживую: загрузка PDF из `data/raw/`, загрузка `.txt` через UI
    (реальный `<input type="file">`, не мок), добавление ссылки на
    Wikipedia — во всех случаях цитаты указывают на верный файл/страницу/
    абзац, числа в цитатах совпадают с оригиналом (напр. «62,4%» из
    тестового файла процитировано без искажений), а вопрос вне корпуса
    документов даёт «нет данных», а не выдумку.
  - `data/rag/` в `.gitignore` (как и `data/raw/`) — загруженный
    пользователем контент не должен попадать в репозиторий.
...
| RAG-чат по загруженным файлам/ссылкам с постраничным цитированием | ✅ реализовано (`backend/rag/`): загрузка pdf/docx/txt и внешних ссылок через UI, ответ строго по найденным фрагментам с `[N]`-цитатами (документ+страница/абзац), явный отказ при отсутствии данных вместо выдумки, confidence по числу/качеству подтверждений — см. раздел выше |
