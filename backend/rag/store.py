...
class DocumentStore:
    ...
    def add_document(
        ...
        """Возвращает (метаданные документа, is_duplicate). Дедупликация — по
        sha256 извлечённого текста (а не сырых байт файла: два файла с одинаковым
        содержимым, но разным форматом/именем, всё равно дают тот же текст для
        поиска, и незачем гонять их через модель эмбеддингов дважды — как и
        манифест в nlp_pipeline/pipeline.py, только для RAG-хранилища, а не графа).
        Если контент уже есть — просто возвращаем существующую запись, не трогая
        эмбеддинги."""
        content_hash = hashlib.sha256("\n".join(b.text for b in blocks).encode("utf-8")).hexdigest()
        existing = next(
            (d for d in self.documents.values() if d.content_hash == content_hash and d.status == "ready"),
            None,
        )
        if existing:
    ...
    def delete_document(self, doc_id: str) -> bool:
        """Убирает документ и все его чанки/эмбеддинги из базы для чата.
        Возвращает False, если такого документа нет (вызывающий код решает,
        что с этим делать — обычно 404)."""
