...
2. **По запросу в UI** — vector search по abstract-чанкам → релевантные документы →
   полный текст перечанкируется и прикрепляется к чату (badge «авто» в
   SourcesPanel). Отбор документов под запрос настраивается (`backend/rag/store.py`):
   - **порог релевантности при активации** — подключаются не «топ-N подряд», а
     только документы со скором ≥ `max(RAG_ACTIVATE_MIN_SCORE,
     RAG_ACTIVATE_TOP_RATIO × лучший_скор)` (динамический отсев под силу запроса),
     чтобы слабо релевантные документы не попадали в «Источники»;
   - **честное обнаружение** — вклад каждого документа в подбор ограничен первыми
     `RAG_DISCOVERY_CHUNK_CAP` чанками (не-статьи индексируются полным текстом, и
     без ограничения один длинный документ выигрывал бы почти любой запрос при
     max-pooling — диагностировано на обзоре про электролит с 30 чанками);
   - **очистка временной базы после запроса** — auto-источники снимаются
     (возврат в `abstract`) сразу по завершении обработки (`finally` в
     `/api/rag/ask` и `/api/rag/ask/stream`), а не только в начале следующего
     запроса, — «следы» предыдущего запроса не накапливаются в контексте чата;
   - вручную загруженные файлы (`attach_source='manual'`) и постоянный индекс не
     затрагиваются.
...
- `GET /api/rag/ask?q=...&auto_attach=true` — auto-attach + grounded ответ + **граф из citation-чанков** + **внешние источники** (`external`: Scholar/Patents)
- `GET /api/rag/ask/stream?q=...` — то же в SSE: этапы конвейера (включая «Внешний поиск») и потоковый ответ в реальном времени
...
Помимо внутренней базы (RAG + загруженные документы) система автоматически ищет
релевантные внешние источники — гибридный поиск (`backend/rag/external_search.py`,
интегрирован в `rag/qa.py` и `rag/stream.py`).

- **Ключевые слова, а не весь вопрос.** После NER из найденного контекста берутся
  ключевые сущности/термины (материалы, процессы, оборудование, предприятия,
  авторы, темы…), ранжируются по числу упоминаний и связности; поисковый запрос
  строится из нескольких самых сильных терминов и их комбинации, при нехватке —
  добор содержательными словами из вопроса.
- **Два источника, параллельно, с таймаутом.** Google Scholar (научные публикации)
  и Google Patents (патенты). Провайдер выбирается автоматически: `SERPAPI_KEY`
  (надёжный шлюз) либо бесплатно без ключа (Patents — неофициальный XHR-JSON,
  Scholar — парсинг HTML).
- **Отказоустойчиво.** Любой сбой сети/капчи/парсинга ловится и превращается в
  пустой результат — ответ по внутренней базе формируется независимо. Если по
  ключевым словам ничего не найдено, показывается сообщение «По данным ключевым
  словам релевантные публикации и патенты не найдены».
- **Раздел «Источники» — три категории:** внутренняя база знаний (RAG/документы),
  Google Scholar, Google Patents. Для каждого внешнего источника: название,
  авторы, год, журнал/конференция или номер патента, краткое описание, ссылка на
  оригинал, ключевые слова находки и оценка релевантности.
- **LLM разделяет происхождение фактов.** В финальном ответе явно помечается, что
  получено из внутренней базы, что подтверждено научными публикациями `[S*]` и что
  патентной литературой `[P*]` (ссылки `[N]` — внутренние фрагменты).
- **Внешние источники — узлы графа.** Найденные публикации/патенты добавляются в
  session-граф как узлы `publication` с `origin=scholar|patent`, связываются рёбрами
  `MENTIONS` с сущностями графа по ключевым словам находки и помечаются на холсте
  бейджем `S`/`P` и цветом контура (легенда — в панели фильтров, метаданные — в
  карточке детали при клике).

Настройки (`.env`): `EXTERNAL_SEARCH_ENABLED`, `EXTERNAL_SEARCH_TIMEOUT`,
`EXTERNAL_SEARCH_MAX`, `SERPAPI_KEY` (опционально).

## UX: переход к результатам

После нажатия «Спросить» страница плавно прокручивается к блоку «По документам»
и переводит на него фокус — пользователь сразу видит ход рассуждений (Thinking) и
последующий потоковый ответ, не ища область результатов вручную.
